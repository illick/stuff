SELECT TOP 2620000 SS.SubscriberKey, SS.EmailAddress, SS.PreferredStore, SS.PriceZone, SS.[Loyalty Number], 'Gmail-Open' AS Domain
FROM [Sendable_subscribers] SS
INNER JOIN [Gmail-Opened Last 30 Days] GO ON SS.SubscriberKey=GO.SubscriberKey
...
AND S.State NOT IN ('TN','KS')

UNION
SELECT TOP 300000 SS.SubscriberKey, SS.EmailAddress, SS.PreferredStore, SS.PriceZone, SS.[Loyalty Number], 'Gmail-Sendable' AS Domain
FROM [Sendable_subscribers] SS
INNER JOIN [GmailDomain_Sendable] GO ON SS.SubscriberKey=GO.SubscriberKey
INNER JOIN [STORE] S ON SS.PreferredStore = S.store_key
LEFT JOIN [global_suppression] GS ON SS.SubscriberKey=GS.SubscriberKey
LEFT JOIN [No Offers] NOFF ON SS.SubscriberKey=NOFF.SubscriberKey
LEFT JOIN [Gmail-Opened Last 30 Days] OP ON SS.SubscriberKey=OP.SubscriberKey
WHERE
GS.SubscriberKey IS NULL
AND NOFF.SubscriberKey IS NULL
AND OP.SubscriberKey IS NULL
AND S.State NOT IN ('TN','KS')
...
UNION
SELECT TOP 21000 SS.SubscriberKey, SS.EmailAddress, SS.PreferredStore, SS.PriceZone, SS.[Loyalty Number], 'MSFT-Click' AS Domain
FROM [Sendable_subscribers] SS
...
SELECT TOP 59000 SS.SubscriberKey, SS.EmailAddress, SS.PreferredStore, SS.PriceZone, SS.[Loyalty Number], 'MSFT-Open' AS Domain
FROM [Sendable_subscribers] SS
...
UNION
SELECT TOP 5000 SS.SubscriberKey, SS.EmailAddress, SS.PreferredStore, SS.PriceZone, SS.[Loyalty Number], 'twc' AS Domain
FROM [Sendable_subscribers] SS
...
AND NOFF.SubscriberKey IS NULL
AND S.State NOT IN ('TN','KS')
AND GO.[EmailAddress] LIKE '%@twc.com'
...
AND NOFF.SubscriberKey IS NULL
AND S.State NOT IN ('TN','KS')
AND GO.[EmailAddress] NOT LIKE '%@twc.com'